...
def dbt_build(pipeline_info: PipelineInfo) -> None:
    ...
    try:
        # здесь должна быть логика dbt build
        subprocess.run(
            [
                'dbt',
                'build',
...
if __name__ == '__main__':
    ...
    rgbi_ingest_info: IngestionMetrics = rgbi_ingestion(pipeline_info=pipeline, dataset_run_info=rgbi_run)
    keyrate_ingest_info: IngestionMetrics = keyrate_ingestion(
        pipeline_info=pipeline,
        dataset_run_info=keyrate_run
    )

    rgbi_stage_info: StagingMetrics = rgbi_staging(
        pipeline_info=pipeline,
        dataset_run_info=rgbi_run,
        ingestion_info=rgbi_ingest_info
    )
    keyrate_stage_info: StagingMetrics = keyrate_staging(
        pipeline_info=pipeline,
        dataset_run_info=keyrate_run,
        ingestion_info=keyrate_ingest_info
    )

    dbt_build(pipeline)

    # [rgbi_finish, keyrate_finish] >> dbt

    finish_dataset_runs(rgbi_run, keyrate_run)
    # dbt >> update_loads

    finish_pipeline_run(pipeline)
    # update_loads >> finish_pipeline
